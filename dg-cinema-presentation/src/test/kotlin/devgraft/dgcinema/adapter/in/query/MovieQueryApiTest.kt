    ...
    fun searchMovie_return_value() {
        ...
        mockMvc.perform(get("/movies/{movieId}", 1))
            ...
            .andDo(document(
                ...
                responseFields(
                    fieldWithPath("movieId").description("영화 아이디"),
                    fieldWithPath("title").description("영화 제목"),
                    fieldWithPath("description").description("영화 설명"),
                    fieldWithPath("bannerUrl").description("영화 배너 URL"),
                    fieldWithPath("genre").description("영화 장르"),
                    fieldWithPath("releaseDate").description("영화 개봉일"),
                    fieldWithPath("duration").description("영화 길이(1분)")
                )
            ))
    }
